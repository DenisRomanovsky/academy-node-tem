...
fn gender() {
	...
	assert_eq!(Kitty([1; 16]).gender(), KittyGender::Male);
	assert_eq!(Kitty([2; 16]).gender(), KittyGender::Male);
	assert_eq!(Kitty([3; 16]).gender(), KittyGender::Male);
	assert_eq!(Kitty([4; 16]).gender(), KittyGender::Male);
	assert_eq!(Kitty([5; 16]).gender(), KittyGender::Female);
	assert_eq!(Kitty([6; 16]).gender(), KittyGender::Female);
	assert_eq!(Kitty([7; 16]).gender(), KittyGender::Female);
	assert_eq!(Kitty([8; 16]).gender(), KittyGender::Female);
	assert_eq!(Kitty([9; 16]).gender(), KittyGender::Female);
}
...
fn can_breed() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(KittiesModule::breed(Origin::signed(100), 0, 11), Error::<Test>::KittenNotFound);
		assert_noop!(KittiesModule::breed(Origin::signed(100), 0, 0), Error::<Test>::SameGenderBreed);
		assert_noop!(KittiesModule::breed(Origin::signed(101), 0, 1), Error::<Test>::KittenNotFound);
		...
		assert_eq!(last_event(), Event::kitties(RawEvent::KittyBreed(100, kitty_one,kitty_two, kitty)));
	});
}
